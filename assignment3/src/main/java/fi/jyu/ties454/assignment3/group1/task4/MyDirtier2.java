 ...
 */
public class MyDirtier2 extends GameAgent {
	...
	protected void setup() {
		...
		this.addBehaviour(new OneShotBehaviour() {
			...
			public void action() {
				Random r = new Random();
				Optional<AreaDirtier> areaDirtier = MyDirtier2.this.getDevice(DefaultDevices.AreaDirtier.class);
				Optional<JumpForwardMover> jumper = MyDirtier2.this.getDevice(DefaultDevices.JumpForwardMover.class);
				if (jumper.isPresent()) {
					jumper.get().move();
					jumper.get().move();
					while (true) {
						...
						while (jumper.get().move() == 5) {
							...
							if (r.nextInt(25) == 0) {
								...
								// there is money left.
								Optional<DirtExplosion> dirtExplosion = MyDirtier2.this
