			...
			public void action() {
				...
				addBehaviour(new Behaviour() {
					...
					public void action() {
						...
						addBehaviour(new Behaviour() {
							...
							public void action() {
								...
								if (msg != null){
									...
									//System.out.println("1--msg.getSender().getLocalName()"+msg.getSender().getLocalName());
									}
										if(msg.getSender().getLocalName().equals("Cleaner2")) {
											Location door = null;
									try {
									...
									//System.out.println("cleaner 1--"+door);
						...
						// map model ends
						//finish one whole search
						System.out.println(" cleaner 1 finish one whole search");
			...
			class searchDoors extends OneShotBehaviour {
						...
						if (firstRun||!t.getLocation().equals(doorOfFirstEntry)) {
							...
							if (distanceLine > 0) {// not read at corner
								...
								if (distanceWall == 0) {// hit wall
									...
									sendMsg(firstDoorOfnewRoom.getLocation(), 2);
									}
									if (door != null) {
				...
				public void moveToNextRoom(ArrayList<doorInfor> doorsOfRoom) {
					...
					for (int i = 0; i < doorsOfRoom.size(); i++) {
						if (doorsAgent2Walked.contains(doorsOfRoom.get(i)
								.getLocation())) {
...
//									.getLocation());
					...
					// record history door
					doorsAgent1Walked.add(doorsOfRoom.get(
							doorsOfRoom.size() - 1).getLocation());
					// send email to 2 which door has been walked
				...
				public void sendMsg(Serializable content, int i) {
					...
					AID dest = null;
					if(i==2){ dest = new AID("Cleaner2", AID.ISLOCALNAME);}
	...
	static class doorInfor implements Serializable {
		private static final long serialVersionUID = 1L;
		Location l;
		Orientation o;

		public doorInfor(Location l, Orientation o) {
			this.l = l;
			this.o = o;
		}
